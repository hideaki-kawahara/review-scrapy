...
== 情報を取得する意味
遠い昔、パソコン通信時代やインターネット初期のころはオートパイロットやRSSリーダーなどのツールがあり、それを利用する個人向けのサービスがありました。これらのツールや個人向けサイトを使用することで必要な情報を必要なときに取得できました。インターネット初期のころ、インターネットを利用するには接続という手順を踏む時代では重宝されたツールでした。
...
このようなテクノロジーはロストテクノロジーになってしまったのでしょうか？@<br>{}
...
下のname属性にrobotsの指定されているmetaタグがあり、contentの指定でindex,followがあるWebサイトでWebスクレイピングを行うようにしてください。また、name属性にrobotsの指定されているmetaタグが存在しないときは、下の指定がされているとみなして良いのでWebスクレイピングしても問題ないです。
...
@<b>{下のrobotsの指定されているmetaタグが1つでも存在するWebサイト注意が必要です！}@<br>{}
ロボット検索を許容していないためWebスクレイピングを行うことは絶対に控えてください。
...
=== robots.txtに関する注意
robots.txtはWebサイトにあるテキストファイルで、スクレイピングに対して指示を出すファイルです。Webサイトでは置いてあることが多いのでスクレイピングする前に確認しておきましょう。下のような記述があればスクレイピングを拒否する設定なのでWebスクレイピングを行うことは絶対に控えてください。
...
=== 過剰なアクセスに関する注意
Webスクレイピングはリクエストの仕方によっては短時間に大量のアクセスをしてしまうことがあり、Webサイトに対して過剰なアクセスをしてしまい迷惑をかけることになります。開発時はスクレイピングのキャッシュを有効にしてできるだけ過剰なアクセスをしないように心がけてください。
