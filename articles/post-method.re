...
2010年ごろは、何でもPOSTメソッドで画面遷移するサイトが存在しました。Strutsフレームワークが代表的で2020年の今も政府系のサイトでは非常に多く使われおりましたが、Struts2の脆弱性問題で少し減りました。
...
次は@<hd>{first-step|Scrapyの設定}を設定し、次はSpiderを作成します。
